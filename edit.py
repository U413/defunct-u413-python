...
def edit_func(args,u413):
	...
	if "step" in u413.cmddata:
		...
		#NEW BODY>
			...
			post=int(db.query("SELECT owner FROM posts WHERE id=%i;"%u413.cmddata["id"]))
			owner=int(db.query("SELECT access FROM users WHERE id=%i;"%post))
			if owner!=u413.user.userid:
				if u413.user.level<user.User.halfmod or u413.user.level<=owner:
					...
					return
			db.query("UPDATE posts SET post='%s',editor=%i,edited=NOW() WHERE id=%i;"%(db.escape(args),u413.user.userid))
			...
			if isint(params[0]):
				...
				if owner!=u413.user.userid:
					if u413.user.level<user.User.halfmod or u413.user.level<=owner:
						u413.type("You do not have permission to edit other user's posts.")
						return
				db.query("UPDATE posts SET post='%s',editor=%i,edited=NOW() WHERE id=%i;"%(db.escape(params[1]),u413.user.userid))
				u413.type("Post edited successfully.")
